...
namespace UniWebServer
{
    ...
    public class WebServer : IDisposable
    {
        ...
        void AcceptConnections ()
        {
            while (true) {
                try {
                    ...
                    if (!stopped) {
                        Debug.Log("Added request '" + tc.Client.Handle + "'" + Thread.CurrentThread.Name);
        ...
        {
            Debug.Log("Processing request '"+ tc.Client.Handle +"'" + Thread.CurrentThread.Name);
            tc.LingerState= new LingerOption(true, 0);
